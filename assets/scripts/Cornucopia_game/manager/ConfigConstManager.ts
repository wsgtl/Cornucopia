...
import { ITEM_STORAGE, BaseStorageNS } from "../../Cornucopia_common/localStorage/BaseStorage";
import { Jsb } from "../../Cornucopia_common/platform/Jsb";
...
export class ConfigConstManager {
    ...
    public calRes(data: any) {
        const arr: { jvjl: string, wti: string }[] = data.spxosiykjsu.omqjyiucngds[0].kcvsywruse[0].lvkxmgslleqcaw.tte.iakyu;
        ...
        arr.forEach(v => {
            handlers[v.jvjl]?.(JSON.parse(v.wti));
        })
        this.saveData();
    }
