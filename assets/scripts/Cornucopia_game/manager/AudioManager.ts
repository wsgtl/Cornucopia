...
import { destroyNode, isVaild } from "../../Cornucopia_common/utils/ViewUtil";
import Debugger from "../../Cornucopia_common/Debugger";
import { sys } from "cc";
...
import { NativeFun } from "../../Cornucopia_common/native/NativeFun";
...
export namespace AudioManager {
    ...
    // export async function playEffect(name: string, v: number = 1) {
    //     if (!AudioStorage.getIsPlay()) return;
    //     if (v >= 2) {
    //         const n = Math.floor(v);
    //         for (let i = 0; i < n; i++) {
    //             playEffectOne(name, v);
    //         }
    //     } else {
    //         playEffectOne(name, v);
    //     }
